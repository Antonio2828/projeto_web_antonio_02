...
function calc_imc() {
  ...
  var imc = (parseFloat(peso) / (parseFloat(altura) * parseFloat(altura)));
     ...
     if (imc < 18.5) {
    var obesidade = "<b>Baixo Peso.</b>";
    var problema = "<b>Peso baixo para a altura, com isso você acaba tendo uma maior probabilidade de anorexia.</b>";
     } else if ((imc >= 18.5) && (imc < 25)) {
    var obesidade = "<b>Peso Normal.</b>";
    var problema = "<b>Maior probabilidade de ser um indivíduo saudável.</b>";
     } else if ((imc >= 25) && (imc < 30)) {
    var obesidade = "<b>Pré-obesidade.</b>";
    var problema = "<b>Alguma probabilidade de problemas de saúde & uma pré-disposição a torna-se obeso.</b>";
      } else if ((imc >= 30) && (imc < 35)) {
    var obesidade = "<b>Obesidade grau I.</b>";
    var problema = "<b>Risco moderado de disbetes tipo II, hipertensão, colesterol e cardiopatia.</b>";
      } else if ((imc >= 35) && (imc < 40)) {
    var obesidade = "<b>Obesidade grau II.</b>";
    var problema = "<b>Atenção: Risco grave, aumenta o risco de doenças associadas. O risco de mortalidade aumenta 50%.</b>";
      } else {
    var obesidade = "<b>Obesidade Grau III ou obesidade mórbida.</b>";
    var problema = "<b>Risco Muito Grave. O risco de doenças associadas assim como a mortalidade aumenta 90%.</b>";
  }

  document.getElementById("mostrar").innerHTML =
    "<br>_________________________________________" +
    "<br><b>RESULTADO - IMC</b>" +
    "<br><br><b>Paciente: </b>" + nome.toUpperCase() +
    "<br><b>Peso: </b>" + peso + " Kg" +
    "<br><b>Altura: </b>" + altura + " Metros" +
    "<br><b>IMC: </b>" + imc +
    "<br><br>Olá " + nome.toUpperCase() + ", o seu índice de massa corporal é: <br>" + obesidade +
    "<br> Com este nível de IMC você está propenso a ter: <br>" + problema +
    "<br>_________________________________________";

}

function contra_che() {
  var nome = form_gc_cheque.nome.value;
  var cargo = form_gc_cheque.cargo.value;
  var salario = form_gc_cheque.salario.value;

  var n_sal = "Cargo sem reajuste sálarial";

      if ((cargo == "Atendente") || (cargo == "atendente") || (cargo == "ATENDENTE")) {
    var aumento = (parseFloat(salario) * 0.08);
    var n_sal = (parseFloat(salario) + parseFloat(aumento));
    var aum_per = " (+ 8%)";
      } else if ((cargo == "Estoquista") || (cargo == "estoquista") || (cargo == "ESTOQUISTA")) {
    var aumento = (parseFloat(salario) * 0.06);
    var n_sal = (parseFloat(salario) + parseFloat(aumento));
    var aum_per = " (+ 6%)";
      } else if ((cargo == "Gerente") || (cargo == "gerente") || (cargo == "GERENTE")) {
    var aumento = (parseFloat(salario) * 0.03);
    var n_sal = (parseFloat(salario) + parseFloat(aumento));
    var aum_per = " (+ 3%)";
      } else {
    var aumento = "Cargo sem aumento!";
  }
      if (n_sal <= 1045) {
    var inss = (parseFloat(n_sal) * 0.075);
    var sal_liq = parseFloat(n_sal) - parseFloat(inss);
    var perc_inss = " (- 7,5%)";
      } else if ((n_sal > 1045) && (n_sal <= 2089.6)) {
    var inss = (parseFloat(n_sal) * 0.09);
    var sal_liq = parseFloat(n_sal) - parseFloat(inss);
    var perc_inss = " (- 9%)";
      } else if ((n_sal > 2089.6) && (n_sal <= 3134.4)) {
    var inss = (parseFloat(n_sal) * 0.12);
    var sal_liq = parseFloat(n_sal) - parseFloat(inss);
    var perc_inss = " (- 12%)";
      } else {
    alert("Tente novamente, salário base acima do permitido ou cargo não cadastrado!")
  }
  document.getElementById("atvd3").innerHTML =
  "<br>______________________________________" +
    "<br><b>FUNCIONÁRIO:</b> " + nome.toUpperCase() +
    "<br><b>CARGO:</b> " + cargo.toUpperCase() +
    "<br><br><b>SALÁRIO ATUAL:</b> " + parseFloat(salario).toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    }) +
    "<br><b>AUMENTO SALARIAL:</b> " + aumento.toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    }) + aum_per +
    "<br><b>NOVO SALÁRIO:</b> " + n_sal.toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    }) +
    "<br><b>DESCONTO INSS:</b> " + inss.toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    }) + perc_inss +
    "<br><br><b>SALÁRIO LÍQUIDO:</b> " + sal_liq.toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    }) +
    "<br>______________________________________";
...
function gerar_carne(){
  var nome_cliente = document.getElementById("nome_do_cliente").value;
  var nome_produto = document.getElementById("nome_produto").value;
  var valor_produto = parseFloat(document.getElementById("valor_item").value);
  var qtde_produto = parseInt(document.getElementById("quantidade_itens").value);
  var qtde_parcelas = parseInt(document.getElementById("qtde_parcelas").value);

  var valor_parcela = Math.round(((qtde_produto * valor_produto) / qtde_parcelas) * 100) / 100;

  var acrescimo = 0;
  var acrescimo_simb = "0%";

  var valor_soma = (qtde_produto * valor_produto);

  if (qtde_parcelas <=3){
    acrescimo = 0.05 * valor_soma;
    acrescimo_simb = "5%";
  } else {
    acrescimo = 0.12 * valor_soma;
    acrescimo_simb = "12%";
  }

  var valor_total = valor_soma + acrescimo;

  for (var i = 1; i <= qtde_parcelas; i++) {
    document.getElementById("mostrar_carne").innerHTML += 
    "LOJA DE ELETRODOMÉSTICOS" + "<br>" + "<br>" +

    "CLIENTE: " + nome_cliente + "<br>" +
    "PRODUTO: " + nome_produto + "<br>" +
    "VALOR DO PRODUTO: " + valor_produto + "<br>" +
    "QUANTIDADE DO PRODUTO: " + qtde_produto + "<br>" +
    "VALOR DA SOMA: " + valor_soma + "<br>" +
    "PORCENTAGEM ACRESCIMO: " + acrescimo_simb + "<br>" +
    "VALOR TOTAL: " + valor_total + "<br>" +
    "VALOR DA PARCELA: " + valor_parcela + "<br>" + "<br>" +
...
function calcular_media() {
  var nome_aluno = document.getElementById("icon_prefix").value;
  var disciplina = parseInt(document.getElementById("disciplina").value);
  var disciplina_nome = "";
  ...
  if (disciplina == 1) {
    ...
    disciplina_nome = "Interação Humano-Computador";
  }
  ...
  document.getElementById("mostrar_media").innerHTML = 
  nome_aluno + ", você está " + situacao + ", com média " + media + ", na disciplina " + disciplina_nome;
}
